...
class MOTR(nn.Module):
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last, run_mode='supervised'):  # the post-processing of training
        ...
        # Introduce the action sampling logic of RL
        if run_mode == 'sampling':
            logits = frame_res['pred_logits'][0, :, 0]
            probs = logits.sigmoid()  # dim = -1
            m = torch.distributions.Bernoulli(probs)
            action = m.sample()
            log_prob = m.log_prob(action)  # for subsequent 'Advantage' calculation in GRPO
            frame_res['log_prob'] = log_prob
            track_scores = action
        else:
            with torch.no_grad():
                if self.training:
