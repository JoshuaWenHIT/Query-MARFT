...
# ------------------------------------------------------------------------
# MOT20 submission/inference script, mirroring submit_dance.py.
#
# Differences vs. submit_dance.py:
#   * iterates MOT20/test instead of DanceTrack/test
#   * works with 6-digit frame filenames (000001.jpg)
#   * emits MOT Challenge 10-column result rows (<frame>,<id>,x,y,w,h,1,-1,-1,-1)
# ------------------------------------------------------------------------
...
class ListImgDataset(Dataset):
    def __init__(self, mot_path, img_list, det_db) -> None:
        ...
        self.img_height = 800
        self.img_width = 1536
        self.mean = [0.354, 0.318, 0.310]
        self.std = [0.221, 0.219, 0.212]
    ...
    def load_img_from_file(self, f_path):
        ...
        # .get() so sequences without entries in det_db still run (proposals become empty).
        for line in self.det_db.get(f_path[:-4] + '.txt', []):
...
class Detector(object):
    ...
    def detect(self, prob_threshold=0.6, area_threshold=100, vis=False):
        ...
        with open(os.path.join(self.args.mot_path, self.args.det_db)) as f:
            det_db = json.load(f)
        loader = DataLoader(ListImgDataset(self.args.mot_path, self.img_list, det_db),
                            1, num_workers=2)
        lines = []
        for i, data in enumerate(tqdm(loader)):
            ...
            res = self.detr.inference_single_image(cur_img, (seq_h, seq_w),
                                                   track_instances, proposals)
            ...
            for xyxy, track_id in zip(bbox_xyxy, identities):
                ...
                x1, y1, x2, y2 = xyxy
                w, h = x2 - x1, y2 - y1
                lines.append(save_format.format(frame=i + 1, id=track_id,
                                                x1=x1, y1=y1, w=w, h=h))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('MOTRv2 MOT20 submission', parents=[get_args_parser()])
    parser.add_argument('--score_threshold', default=0.5, type=float)
    parser.add_argument('--update_score_threshold', default=0.5, type=float)
    parser.add_argument('--miss_tolerance', default=20, type=int)
    parser.add_argument('--split', default='test', choices=['train', 'test'],
                        help='Which MOT20 split to run (train = for validating on train sequences).')
    ...
    detr.track_base = RuntimeTrackerBase(args.score_threshold, args.score_threshold,
                                         args.miss_tolerance)
